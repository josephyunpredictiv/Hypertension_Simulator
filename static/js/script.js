...
document.addEventListener("DOMContentLoaded", function () {
    ...
    const positions = {
        ...
        유산소운동일Input: { left: baseLeft - leftscale, top: baseTop + topscale + 4 * inputSpacing, width: inputWidth, height: inputHeight },
        음주회수Input: { left: baseLeft - leftscale, top: baseTop + topscale + 5 * inputSpacing, width: (inputWidth + 8) / 1.7, height: inputHeight + 6 },
        음주회수Input2: { left: baseLeft - leftscale, top: baseTop + topscale + 5 * inputSpacing, width: inputWidth / 5, height: inputHeight },
        금연여부Input: { left: baseLeft - leftscale, top: baseTop + topscale + 6 * inputSpacing, width: inputWidth + 10, height: inputHeight + 6 },
    };
    ...
    const inputLabels = {
        ...
        체중Input: "체중 (kg)",
        허리둘레Input: "허리둘레 (cm)",
        근력운동일Input: "1주일간 근력 운동일 수",
        유산소운동일Input: "1주일간 중등도 신체활동을 며칠 하십니까?",
        음주회수Input: "술마시는 횟수",
        음주회수Input2: "",
        금연여부Input: "평생 5갑 이상 담배를 핀 적?",
    };
    ...
    for (const [key, { left, top, width, height }] of Object.entries(positions)) {
        ...
        if (key === "음주회수Input" || key === "금연여부Input") {
            ...
            const choices = key === "음주회수Input" ? ['금주', '1년', '한달', '일주일'] : ['아니요', '예'];
...
function addOutputBoxAndImage() {
    ...
    const inputLabels = {
        수축기혈압Output: "수축기혈압 (mmHg)",
        이완기혈압Output: "이완기혈압 (mmHg)",
        체질량지수Output: "체질량지수 (kg/m<sup>2</sup>)",
        중성지방Output: "중성지방 (mg/dL)",
        사구체여과율Output: "사구체여과율 (ml/min)",
        공복혈당Output: "공복혈당 (mg/dL)",
        HDLOutput: "HDL-콜레스테롤 (mg/dL)",
        LDLOutput: "LDL-콜레스테롤 (mg/dL)",
        ALTOutput: "ALT (IU/L)",
        ASTOutput: "AST (IU/L)",
    };
